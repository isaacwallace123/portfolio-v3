...
.PHONY: help up down build rebuild logs ps fresh dev-db dev-auth-service dev-api dev-web dev-admin dev-auth dev-cyberlab check lint fmt test smoke clean
...
up: ## build + start everything (db, api, web, admin, cyberlab)
	...
	@echo "auth portal   http://localhost:3005"
	@echo "auth service  http://localhost:5170/health"
...
dev-auth-service: ## run the auth service natively with hot reload (:5170), against docker Postgres
	cd apps/auth-service && ConnectionStrings__Postgres="Host=localhost;Port=5432;Database=iw;Username=iw;Password=iw-dev-only" dotnet watch --launch-profile http
...
dev-auth: ## run the identity portal natively with hot reload (:3005)
...
clean: ## remove build output
	...
	rm -rf apps/web/.next apps/admin/.next apps/auth/.next ../cyberlab/web/.next
